...
void
gv_fetchmethod_autoload_type(stash, methname, type, autoload, flags)
    HV* stash
    SV* methname
    int type
    I32 autoload
    ...
    PPCODE:
        switch (type) {
           case 0:
	       gv = gv_fetchmethod_autoload(stash, SvPVX_const(methname), autoload);
               break;
           case 1:
               gv = gv_fetchmethod_sv_autoload(stash, methname, autoload, flags);
               break;
           case 2:
               gv = gv_fetchmethod_pv_autoload(stash, SvPV_nolen(methname), autoload, flags | SvUTF8(methname));
               break;
           case 3: {
               STRLEN len;
               const char * const name = SvPV_const(methname, len);
               gv = gv_fetchmethod_pvn_autoload(stash, name, len, autoload, flags | SvUTF8(methname));
               break;
            }
        }
