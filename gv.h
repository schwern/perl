...
#  define GvNAMEWASUTF8(gv)	({ assert(GvNAME_HEK(gv)); HEK_WASUTF8(GvNAME_HEK(gv)); })
#else
...
#  define GvNAMEWASUTF8(gv)	HEK_WASUTF8(GvNAME_HEK(gv))
#endif
...
#define GvENAMEWASUTF8(gv) GvNAMEWASUTF8(GvEGV(gv) ? GvEGV(gv) : gv)
...
#define gv_fetchmethod_autoload(stash,name,autoload) gv_fetchmethod_pv_autoload(stash, name,autoload,0)
